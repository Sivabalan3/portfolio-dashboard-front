import React, { useEffect } from "react";
import { useSelector, useDispatch } from "react-redux";
import {
  getProjectOrder,
  updateProjectOrderContact,
  deleteProjectOrderById,
} from "../store/projectstores/projectSlice";
import "./styles/loder.css";
import { notification } from "antd";
import { MdAutoDelete } from "react-icons/md";

const OrderTable = () => {
  const dispatch = useDispatch();
  const {
    data: orders,
    loading,
    error,
  } = useSelector((state) => state.projects.getProjectOrder);
  const { userInfo } = useSelector((state) => state.auth);
  useEffect(() => {
    dispatch(getProjectOrder());
  }, [dispatch]);
  const handleToggleContact = (order) => async () => {
    // Toggle the contact value
    if (userInfo.isAdmin) {
      const newContactValue = !order.contact;

      // Dispatch the update action
      dispatch(
        updateProjectOrderContact({
          orderId: order._id,
          updatedOrderData: { contact: newContactValue },
        })
      );
      notification.success({
        message: "Success",
        description: "Contact  updated to Admin successfully!",
      });
    } else {
      notification.error({
        message: "Error",
        description: "You do not have permission to changes...",
      });
    }
  };
  if (loading === "loading") {
    return <div className="cube">
    <div className="cube_item cube_x"></div>
    <div className="cube_item cube_y"></div>
    <div className="cube_item cube_y"></div>
    <div className="cube_item cube_x"></div>
  </div>;
  }

  if (loading === "failed") {
    return <div>Error: {error}</div>;
  }
  return (
    <>
      <div className="relative overflow-x-auto shadow-md sm:rounded-lg mb-24 mt-12">
        <div className="flex items-center justify-between flex-column flex-wrap md:flex-row space-y-4 md:space-y-0 pb-4 bg-white dark:bg-gray-900">
          <div></div>
          <label htmlFor="table-search" className="sr-only">
            Search
          </label>
          <div className="relative pt-4">
            <div className="mt-4  absolute inset-y-0 rtl:inset-r-0 start-0 flex items-center ps-3 pointer-events-none">
              <svg
                className="w-4 h-4 text-gray-500 dark:text-gray-400"
                aria-hidden="true"
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 20 20"
              >
                <path
                  stroke="currentColor"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  strokeWidth="2"
                  d="m19 19-4-4m0-7A7 7 0 1 1 1 8a7 7 0 0 1 14 0Z"
                />
              </svg>
            </div>
            <input
              type="text"
              id="table-search-users"
              className="mr-4 block p-2 ps-10 text-sm text-gray-900 border border-gray-300 rounded-lg w-80 bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
              placeholder="Search for users"
            />
          </div>
        </div>
        <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
          <thead className="text-md text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
            <tr>
              <th scope="col" className="px-6 py-3">
                Name
              </th>
              <th scope="col" className="px-6 py-3">
                Gmail
              </th>
              <th scope="col" className="px-6 py-3">
                Phone
              </th>
              <th scope="col" className="px-6 py-3">
                Date
              </th>
              <th scope="col" className="px-6 py-3">
                Time
              </th>
              <th scope="col" className="px-6 py-3">
                Address
              </th>
              <th scope="col" className="px-6 py-3">
                Order Status
              </th>
              <th scope="col" className="px-6 py-3">
                Delete
              </th>
            </tr>
          </thead>
          <tbody>
            {orders.map((order, index) => (
              <tr
                key={order._id}
                className="text-[14px] bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600"
              >
                <th
                  scope="row"
                  className="flex items-center px-6 py-4 text-gray-900 whitespace-nowrap dark:text-white"
                >
                  {/* <img className="w-10 h-10 rounded-full" src="/docs/images/people/profile-picture-1.jpg" alt="Jese image"/> */}
                  <div className="ps-3">
                    <div className="text-base font-semibold text-violet-500">
                      {order.username.toUpperCase()}
                    </div>
                    <div className="font-normal dark:text-amber-300">
                      {order.useremail}
                    </div>
                  </div>
                </th>
                <td className="px-6 py-4 dark:text-fuchsia-500">
                  {order.useremail}
                </td>
                <td className="px-6 py-4 dark:text-amber-500">
                  {order.userphone}
                </td>
                <td className="px-6 py-4 dark:text-pink-500">
                  {new Date(order.userdate).toLocaleDateString("en-GB", {
                    day: "numeric",
                    month: "long",
                    year: "numeric",
                  })}
                </td>
                <td className="px-6 py-4 dark:text-lime-500">
                  {order.usertime}
                </td>
                <td className="px-6 py-4 dark:text-yellow-300">
                  {order.useraddres}
                </td>
                <td className="px-6 py-4">
                  <div className="flex items-center">
                    <div
                      className={
                        order.contact === false
                          ? "h-2.5 w-2.5 bg-red-500 rounded-full  me-2"
                          : ""
                      }
                    ></div>
                    Not
                    <label className="inline-flex items-center me-5 cursor-pointer ps-6">
                      <input
                        type="checkbox"
                        checked={order.contact}
                        onChange={handleToggleContact(order)}
                        className="sr-only peer"
                      />

                      <div className="relative w-11 h-6 bg-gray-200 rounded-full peer dark:bg-gray-700 peer-focus:ring-4 peer-focus:ring-purple-300 dark:peer-focus:ring-purple-800 peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-0.5 after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-purple-600"></div>
                      <span
                        className={
                          order.contact === false
                            ? "ms-3 text-sm font-medium text-gray-900 dark:text-blue-500"
                            : "ms-3 text-sm font-medium text-gray-900 dark:text-green-500"
                        }
                      >
                        Is Confirmed
                      </span>
                    </label>
                  </div>
                </td>
                <td className="px-6 py-4">
                  <button
                    type="button"
                    onClick={() => {
                      if (userInfo.isAdmin) {
                        dispatch(
                          deleteProjectOrderById({ orderId: order._id })
                        );
                      } else {
                        notification.error({
                          message: "Error",
                          description: "Only admins can delete users.",
                        });
                      }
                    }}
                    className="flex gap-1 text-purple-700 hover:text-white border border-purple-700 hover:bg-purple-800 focus:ring-4 focus:outline-none focus:ring-purple-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2 dark:border-purple-400 dark:text-purple-400 dark:hover:text-white dark:hover:bg-purple-500 dark:focus:ring-purple-900"
                  >
                    <MdAutoDelete className="w-5 h-5 mt-1" />
                    Delete
                  </button>
                </td>
              </tr>
            ))}
          </tbody>
        </table>
      </div>
    </>
  );
};

export default OrderTable;
